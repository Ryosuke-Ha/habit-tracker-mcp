 ...
 */
export function getJSTDateAfterDays(days) {
    const now = new Date();
 ...
 */
export function getJSTTomorrow() {
 ...
 */
export function ceilTimeSeconds(totalSeconds) {
    const intervalSeconds = ROUND_INTERVAL_MINUTES * 60;
    const ceiledSeconds = totalSeconds % intervalSeconds === 0
        ? totalSeconds
 ...
 *   切り上げた時刻（HH:MM）を返す。
 */
export function ceilToNextSlot(time) {
    if (time != null && time !== '') {
        return time;
    }
    const now = new Date();
