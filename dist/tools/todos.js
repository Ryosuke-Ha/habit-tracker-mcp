...
export const addTodoSchema = z.object({
    title: z.string().trim()
        ...
        .refine((val) => val.length <= LIMITS.TITLE_MAX, (val) => ({
    ...
    location: z.string().trim()
        .refine((val) => val.length <= LIMITS.LOCATION_MAX, (val) => ({
...
export const registerTodoTools = (server) => {
    ...
    server.tool("add_todo", "今日の習慣ログにTODOを追加する。毎日繰り返す習慣・今日やることに使う。特定日の予定（歯医者・会議など）はadd_scheduled_todoを使うこと。", addTodoSchema.shape, async ({ title, scheduled_time, location }) => {
    ...
    server.tool("complete_todo", "TODOを完了にする", completeTodoSchema.shape, async ({ todo_id }) => {
        // 強制層
    ...
    server.tool("add_scheduled_todo", "特定の日付のTODOメモを追加する。歯医者・会議など特定日の予定タスクに使う。毎日繰り返す習慣・今日やることへの追加はadd_todoを使うこと。", addScheduledTodoSchema.shape, async ({ title, scheduled_date, scheduled_time, location, notification_offset_1, notification_offset_2 }) => {
        // 強制層
        if (title.length > LIMITS.TITLE_MAX) {
            return validationError(`[title] 上限${LIMITS.TITLE_MAX}文字を超えています（入力${title.length}文字）。${LIMITS.TITLE_MAX}文字以内に短縮して再実行してください。`);
        }
    ...
    server.tool("add_persistent_todo", "持ち越しTODOを追加する", addPersistentTodoSchema.shape, async ({ title, scheduled_time }) => {
        // 強制層
        if (title.length > LIMITS.TITLE_MAX) {
            return validationError(`[title] 上限${LIMITS.TITLE_MAX}文字を超えています（入力${title.length}文字）。${LIMITS.TITLE_MAX}文字以内に短縮して再実行してください。`);
        }
