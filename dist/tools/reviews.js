...
// ツール登録
// ──────────────────────────────────────────────────────────────────────────────
export const registerReviewTools = (server) => {
    server.tool("get_weekly_summary", "今週の振り返りデータを取得する（達成率・KPT）", {}, async () => {
        const data = await apiRequest("GET", "/reviews/weekly");
        return {
            content: [{ type: "text", text: JSON.stringify(data, null, 2) }],
        };
    });
    server.tool("add_kpt_item", "今週のKPTにアイテムを追加する", addKptItemSchema.shape, async ({ type, content }) => {
        // 強制層
    ...
    server.tool("get_monthly_stats", "今月の達成率・streakを取得する", {}, async () => {
